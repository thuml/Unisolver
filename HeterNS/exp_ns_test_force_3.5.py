...
test_data = np.load(os.path.join(args.data_path, 'results_save_200/ns_vis_nu1e-05_m_3.5_u_200.npy'))
...
def get_force():
    f = torch.zeros([2,s1,s2])
    t = torch.linspace(0, 1, s1+1, device=device)
    t = t[0:-1]
    X,Y = torch.meshgrid(t,t)
    f[0] = 0.1*(torch.sin(1.5*np.pi*(X + Y)) + torch.cos(1.5*np.pi*(X + Y)))
    f[1] = 0.1*(torch.sin(2.5*np.pi*(X + Y)) + torch.cos(2.5*np.pi*(X + Y)))
                
    return f



vis = torch.tensor([0.00001]).to(device)
force = get_force()
...
f = np.load(os.path.join(args.data_path, 'results_save_200/ns_vis_nu1e-05_m_3.5_f_200.npy'))
...
with torch.no_grad():
    for xx, f, yy in test_loader:
        ...
        for t in range(0, T_out, step):
            ...
            plt.subplot(1, 3, 1)
            plt.imshow(im[0,:, :, -1].detach().cpu().numpy())
            plt.subplot(1, 3, 2)
            plt.imshow(y[0, :, :, -1].detach().cpu().numpy())
            plt.subplot(1, 3, 3)
            plt.imshow(abs(im - y)[0, :, :, -1].detach().cpu().numpy())
            plt.savefig(path + args.model + '_' + str(index) + '_' + str(t))
            plt.clf()
